 ...
 * ║   Board  : Arduino Mega 2560 (Wokwi + PlatformIO)           ║
 * ╚══════════════════════════════════════════════════════════════╝
 *
...
// ──────────────────────────────────────────────────────────────────────
//  THRESHOLDS  (rule engine parameters — all configurable)
// ──────────────────────────────────────────────────────────────────────
...
#define SENSOR_PERIOD_MS  2000UL   // ms  — sensor polling interval
...
void setDevice(int pin, bool &state, bool target, const char* device, const char* reason);
...
void setup() {
  ...
  digitalWrite(PIN_DOOR,      HIGH);   // HIGH = locked
  ...
  mqttPublish("system/status",
    ...
    "\"status\":\"ONLINE\","
    "\"sensors\":[\"PIR\",\"DHT22\",\"MQ135\",\"MQ2\",\"FC37\",\"RFID\"]}");
...
void checkPIR(unsigned long now) {
  ...
  if (room.occupied && (now - room.lastMotion > INACTIVITY_MS)) {
    room.occupied = false;
    mqttPublish("sensor/occupancy",
      "{\"room\":\"101\",\"occupied\":false,"
      "\"reason\":\"inactivity_timeout\",\"timeout_s\":20}");
  }
}
...
void sensorCycle() {
  ...
  int co2ppm   = map(analogRead(PIN_CO2),   0, 1023, 400, 2000);
  ...
  if (!room.alertActive) updateLCD(temp, hum, co2ppm);
}
...
void applyRules(float temp, int co2, int smoke, int flood) {
  ...
  // RULE 2: Fan = occupied AND (temp high OR CO2 high)
...
// Only toggle and publish if state actually changes (avoids MQTT spam)
...
// ══════════════════════════════════════════════════════════════════════
//  MQTT BRIDGE — Serial output read by Raspberry Pi Python gateway
// ══════════════════════════════════════════════════════════════════════
void mqttPublish(const char* topic, String payload) {
